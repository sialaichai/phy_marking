...
# ---- Generic: Update password for any user ----
...
def delete_mark_scheme(scheme_id: int):
    supabase = get_supabase()
    supabase.table("mark_schemes").delete().eq("id", scheme_id).execute()

# ---- Submissions (tied to class) ----
def add_submission(mark_scheme_id: int, class_id: int, student_name: str, student_email: str, transcribed_text: str, grade: int, feedback: str, is_trial: bool = True):
    supabase = get_supabase()
    data = {
...
def get_submissions_by_scheme(scheme_id: int):
    supabase = get_supabase()
    result = supabase.table("submissions").select("*, classes(class_name)").eq("mark_scheme_id", scheme_id).order("graded_at", desc=True).execute()
    return result.data

def get_submissions_by_class(class_id: int):
    supabase = get_supabase()
    result = supabase.table("submissions").select("*, mark_schemes(assignment_name, question, total_points)").eq("class_id", class_id).order("graded_at", desc=True).execute()
    return result.data

def get_submissions_by_email(class_id: int, student_email: str):
    supabase = get_supabase()
    result = supabase.table("submissions").select("*, mark_schemes(assignment_name, question, total_points)").eq("class_id", class_id).eq("student_email", student_email).order("graded_at", desc=True).execute()
    return result.data
